...
type SearchFormProps = React.DetailedHTMLProps<
...
export const SearchForm = ({
  ...
  return (
    <form
      className={twMerge(
        classNames(
          'search-form relative flex justify-between items-center rounded-full border border-gray-200 bg-white',
    ...
    >
      ...
      <input
        {...attributes}
        ref={ref}
        className={twMerge(classNames('outline-none w-full p-8 rounded-full text !text-gray'), attributes.inputClassName)}
        type="text"
        value={value}
        onChange={(e) => {
      ...
      <span
        className={`text-gray-800 my-auto mx-12 text-xs ${value ? 'block cursor-pointer' : 'hidden'}`}
        onClick={() => {
          clearValue()
          attributes?.onChange?.('')
        }}
